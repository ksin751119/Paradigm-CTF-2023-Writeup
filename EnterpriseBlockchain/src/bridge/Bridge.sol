...
contract Bridge {
    ...
    mapping(uint256 => mapping(address => bool))
        public isTokenRegisteredAtRemote;
    ...
    modifier onlyRemoteBridge() {
        require(
            msg.sender == address(this) &&
                ...
                remoteBridgeChainId[relayedMessageSenderAddress] ==
                relayedMessageSenderChainId, // A: bridge address => chainId, relayedMessageSenderAddress == msg.sender
            "RB"
        );
        _;
    }

    function registerRemoteBridge(
    ...
    function ethOut(address _to) public payable virtual {
        ...
        this.sendRemoteMessage{value: msg.value}(
            _remoteChainId,
            _remoteBridge,
            abi.encodeWithSelector(Bridge.ethIn.selector, _to)
        );
    }
    ...
    function ERC20Register(
        ...
        if (remoteTokenToLocalToken[_remoteToken] == address(0)) {
            address _token = address(
                new BridgedERC20(_remoteToken, _name, _symbol)
            );
    ...
    function relayMessage(
        ...
        (bool success, bytes memory result) = _targetAddress.call{
            value: _value
        }(_message);
